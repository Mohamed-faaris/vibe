...
import { LotItem } from '../classes/index.js';
...
class AttemptController {
  ...
  async attempt(
    ...
    const filtered = attempt.questionRenderViews.map(view => {
      const viewWithLot = view as any;
      return {
        ...view,
        lotItems: viewWithLot.lotItems?.map((lot: any) => ({ ...lot, explaination: null })),
      };
    });

    return {
      attemptId: attempt.attemptId,
      userAttempts: attempt.userAttempts,
      questionRenderViews: filtered,
